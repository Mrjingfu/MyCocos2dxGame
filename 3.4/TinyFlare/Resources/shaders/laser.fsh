...
void main()
{
    ...
    float deep3;
    float curveX2=0.05*(curPoint.y-0.5*u_spriteSize.y)*(curPoint.y-0.5*u_spriteSize.y)+10.0;//-30.0;
    if(curPoint.x>curveX2){
        deep3=1.0;
    }else{//curPoint.x<=curveX2
        deep3=1.0-1.0/(curveX2)*(curveX2 - curPoint.x);
    }
