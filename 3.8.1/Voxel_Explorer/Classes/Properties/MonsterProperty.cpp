...
void MonsterProperty::adjustByDC()
{
    ...
    m_nAddedMinAttack = m_nAddedMinAttack + (nodeDepth-1)*5 + (int)(currentDepth.GetLongValue()) + (m_nLevel-1)*0.25f;
    m_nAddedMaxAttack = m_nAddedMaxAttack + (nodeDepth-1)*10 + (int)(currentDepth.GetLongValue()*1.5f) + (m_nLevel-1)*0.5f;
    
    m_nAttackDiceNum = m_nAttackDiceNum + (nodeDepth-1);
    m_nAttackDiceFaceNum = m_nAttackDiceFaceNum + (nodeDepth-1)*2 + (m_nLevel-1)*0.5f;
