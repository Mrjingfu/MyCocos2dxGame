...
TerrainTile* TerrainTile::create(TileType type)
{
    ...
    if (tile && tile->initWithFile("16x16x16.c3b"))
    {
        ...
        if((type == TerrainTile::TT_ENTRANCE) && ((RandomDungeon::getInstance()->getCurrentDungeonNode()->m_nCurrentDepth == 1)||(RandomDungeon::getInstance()->getCurrentDungeonNode()->isBossDepth())))
