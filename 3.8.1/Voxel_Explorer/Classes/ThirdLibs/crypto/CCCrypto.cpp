...
void CCCrypto::md5(void* input, int inputLength, unsigned char* output)
{
    MD5_CTX ctx;
    MD5_Init_ex(&ctx);
    MD5_Update_ex(&ctx, input, inputLength);
    MD5_Final_ex(output, &ctx);
}
...
void CCCrypto::encryptXOR(char* memptr, int memlen)
{
	return xor_encrypt(memptr, memlen);
}

void CCCrypto::decryptXOR(char* memptr, int memlen)
{
