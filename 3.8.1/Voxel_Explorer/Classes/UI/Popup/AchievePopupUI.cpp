...
//    m_pAchieveGridView->addEventListener(CC_CALLBACK_2(ShopPopupUI::selectItemEvent, this));
    ...
    for (int j =0; j<ItemSize; j++) {
        ...
        if (isLanguageEn())
            itemui->setContentSize(cocos2d::Size(m_pAchieveGridView->getContentSize().width-20,50));
        else
    ...
    if (isLanguageEn())
        m_pAchieveMangerLayer = AchieveMangerEnLayerUI::create(m_pAchieveGridView->getInnerContainerSize());
    else
...
void AchievePopupUI::updateAllAchieves()
{
    ...
    for (int i=0; i<allAchieves.size(); i++) {
        ...
        if (achieveProp && itemView)
        {
            ...
            m_pAchieveMangerLayer->addItemAchieve(itemView,type, icon,name,targetDesc,isUnlock,isCommple);
        }
    }
}
